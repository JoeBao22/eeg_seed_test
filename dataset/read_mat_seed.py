...
for feature_type in feature_types:
    for smooth_method_type in smooth_method_types:
        ...
        for i in range(num_of_people):
            print("resolving person {} / {}".format(i+1, len(labels)))
            for trial_path in os.listdir(dir_path):
                if trial_path.startswith(str(i+1) + "_"):  # trial record for the person
                    ...
                    for label_index in range(len(labels)):
                        k = feature_type + "_" + smooth_method_type + str(label_index+1)
                        v = feature2dict[k]
                        ...
                        temp_labels = np.ones((num_of_samples, 1)) * labels[label_index]          
